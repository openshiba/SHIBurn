...
export default function App() {
  const shibContract = useShibContract();
  const { account } = useActiveWeb3React();
  const [data, setData] = useState({ totalSupply: "0", currentBalance: "0" });

  useEffect(async () => {
    if (shibContract !== null) {
      let totalSupply = await shibContract.totalSupply();
      let currentBalance = await shibContract.balanceOf(account);
      ...
      <Web3ReactManager>
        ...
        >
          <div className="u-clearfix u-sheet u-sheet-1">
            ...
            <Burner currentBalance={data.currentBalance} />
            <img
